...
//            Spacer() // Push content up
        ...
        .onAppear {
            print("[ADBView] onAppear: starting auto-detect")
            viewModel.startAutoDetect()
            startSearchWindow()
            print("[ADBView] onAppear: search window started")
        }
        .onDisappear {
            print("[ADBView] onDisappear: cancelling detection")
        ...
        .toolbar {
            ToolbarItem(placement: .primaryAction) {
                Button {
                    print("[ADBView] Toolbar refresh tapped")
                    viewModel.startAutoDetect()
                    startSearchWindow()
                } label: {
    ...
    private var emptyStateView: some View {
        VStack(spacing: 20) {
            ...
            Button {
                print("[ADBView] Continue tapped (disabled)")
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if viewModel.devices.count == 1 {
                ...
                Text(single)
                    .font(.body)
                    .onAppear {
                        print("[ADBView] Auto-selecting single device: \(single)")
            ...
            Button {
                print("[ADBView] Primary continue tapped; selected=\(String(describing: viewModel.selectedDevice))")
            ...
            .alert("App will reboot your device into bootloader. You will have to flash a custom recovery of your choice tht is made for your device in order to flash a custom ROM. For a GSI, you will need a GSI of your choice. If your ROM has got individual images, the app will handle them. Continue?", isPresented: $showConfirmReboot) {
                ...
                Button("Continue", role: .destructive) {
                    print("[ADBView] Alert confirmed; rebooting device")
                    if let serial = viewModel.selectedDevice {
                        print("[ADBView] Reboot serial=\(serial) mode=\(isSamsungDevice(serial: serial) ? "download" : "bootloader")")
                        if isSamsungDevice(serial: serial) {
                            reboot(serial: serial, mode: "download")
                        } else {
                            reboot(serial: serial, mode: "bootloader")
                        }
                        print("[ADBView] Navigating to .fastboot")
            ...
            Menu {
                Button {
                    print("[ADBView] Menu: Reboot to Recovery tapped for serial=\(String(describing: viewModel.selectedDevice))")
                    if let serial = viewModel.selectedDevice {
                ...
                Button {
                    print("[ADBView] Menu: Normal Reboot tapped for serial=\(String(describing: viewModel.selectedDevice))")
                    if let serial = viewModel.selectedDevice {
                ...
                Button {
                    print("[ADBView] Menu: Reboot to Bootloader tapped for serial=\(String(describing: viewModel.selectedDevice))")
                    if let serial = viewModel.selectedDevice {
                        reboot(serial: serial, mode: "bootloader")
                    }
                } label: {
                ...
                Button {
                    print("[ADBView] Menu: Reboot to Download Mode tapped for serial=\(String(describing: viewModel.selectedDevice))")
                    if let serial = viewModel.selectedDevice {
                        reboot(serial: serial, mode: "download")
                    }
                } label: {
                    if isMacOS26OrNewer {
                        Label("(Samsung-only) Download mode", systemImage: "arrow.down")
                    } else {
                        Text("(Samsung-only) Download mode")
                    }
                }
            } label: {
    ...
    private func startSearchWindow() {
        print("[ADBView] startSearchWindow: entered")
        isSearching = true
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 10) {
            ...
            print("[ADBView] startSearchWindow: timeout reached; devices empty? \(viewModel.devices.isEmpty)")
            if viewModel.devices.isEmpty {
                isSearching = false
            }
        }
    ...
    private func runADB(serial: String, args: [String]) -> Int32 {
        ...
        print("[ADBView] runADB: exec=\(adbURL.path) args=\(fullArgs.joined(separator: " "))")
        ...
        do {
            ...
            if let output = String(data: data, encoding: .utf8), !output.isEmpty {
                print("[ADBView] runADB output: \n\(output)")
            }
            return process.terminationStatus
        } catch {
            print("[ADBView] runADB error: \(error.localizedDescription)")
            return -1
        }
    }

    private func reboot(serial: String, mode: String?) {
        print("[ADBView] reboot: serial=\(serial) mode=\(String(describing: mode))")
        var arguments = ["reboot"]
        if let mode, !mode.isEmpty {
            arguments.append(mode)
        }
        let realSerial = serial.extractSerial()
        print("[ADBView] reboot: extracted serial=\(realSerial)")
        _ = runADB(serial: realSerial, args: arguments)
    }

    private func isSamsungDevice(serial: String) -> Bool {
        print("[ADBView] isSamsungDevice? input=\(serial)")
        ...
        if let detector = (viewModel as AnyObject) as? SamsungDetecting {
            let result = detector.isSamsung(serial: serial)
            print("[ADBView] isSamsungDevice (detector) -> \(result)")
            return result
        }
        ...
        let result = lower.contains("samsung") || lower.hasPrefix("sm-")
        print("[ADBView] isSamsungDevice (heuristic) -> \(result)")
        return result
    }
}
...
final class ADBDevicesViewModel: ObservableObject {
    ...
    func startAutoDetect() {
        print("[ADBDevicesViewModel] startAutoDetect called")
        ...
        detectTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                print("[ADBDevicesViewModel] polling for devices… current=\(self.devices.count)")
                await self.performRefreshOnce()
                if !self.devices.isEmpty {
                    print("[ADBDevicesViewModel] devices found: \(self.devices.count)")
                    break
                }
                // Check timeout (10 seconds)
                let elapsed = clock.now - start
                if elapsed >= .seconds(10) {
                    print("[ADBDevicesViewModel] timeout reached")
                    break
                }
    ...
    private func performRefreshOnce() async {
        print("[ADBDevicesViewModel] performRefreshOnce")
        ...
        case .success(let list):
            ...
            print("[ADBDevicesViewModel] parsed/mapped devices: \(mapped)")
        ...
        case .failure:
            print("[ADBDevicesViewModel] runADBDevices failed")
            await MainActor.run {
                self.devices = []
    ...
    static func runShellCommand(_ command: [String]) async -> Result<String, Error> {
        print("[ADBDevicesViewModel] runShellCommand: \(command.joined(separator: " "))")
        return await withCheckedContinuation { (continuation: CheckedContinuation<Result<String, Error>, Never>) in
            ...
            guard let executable = command.first else {
                let err = NSError(domain: "RunShellCommand", code: -1, userInfo: [NSLocalizedDescriptionKey: "Empty command"])
            ...
            process.terminationHandler = { proc in
                ...
                print("[ADBDevicesViewModel] runShellCommand exit=\(proc.terminationStatus) output=\n\(output)")
    ...
    private static func parseADBList(_ output: String) -> [String] {
        print("[ADBDevicesViewModel] parseADBList input=\n\(output)")
        let lines: [String] = output
            .split(separator: "\n")
            .map { String($0).trimmingCharacters(in: .whitespacesAndNewlines) }

        let serials: [String] = lines
        ...
        print("[ADBDevicesViewModel] parseADBList serials=\(serials)")
        return serials
    }

    static func readKnownFile() -> [String: String] {
        print("[ADBDevicesViewModel] readKnownFile loading KnownDeviceCodenames.txt")
        guard let url = Bundle.main.url(forResource: "KnownDeviceCodenames", withExtension: "txt"),
              let text = try? String(contentsOf: url, encoding: .utf8)
        else { 
            print("[ADBDevicesViewModel] readKnownFile entries=0")
            return [:] 
        }
        ...
        print("[ADBDevicesViewModel] readKnownFile entries=\(map.count)")
        return map
    }
}
